...
from plate_ocr import detect_best_plate, draw_detection, ocr_gemini, ocr_tesseract, preprocess_variants_for_ocr
...
def run_camera_mode(
    *,
    ...
    cap = st.camera_input("Tire uma foto")
    if not cap:
        st.info("Use a câmera para tirar uma foto.")
        return parking_spots

    try:
        img_bgr = bgr_from_uploaded(cap.getvalue())
